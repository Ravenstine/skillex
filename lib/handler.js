...
module.exports = (states) => {
  ...
  return (request, ctx, callback) => {
    // we default to the entrypoint state if no state is specified in the request
    recall(request)
    .then(request => generateContext(request, {}, states))
    .then(context => {
      return runtime(context, states) // evaluate the request
