...
module.exports = (pillsDirectory) => {
  ...
  return (request, ctx, callback) => {
    ...
    remember(request).then(() => {
      ...
      let hasQuestion      = currentLabel.hasOwnProperty('ask');    

      // kind of a hack at the moment to allow the
      // label's `go to:` to continue to work in case
      // a reply did not specify a `go to:`.
      // not sure where eactly this should end up.
      if(hasQuestion){
        intents(currentLabel.intents, context);
        if(currentLabel.hasOwnProperty('swallow pill')   && !context.sessionAttributes.hasChanged('PILL')){
          context.sessionAttributes.PILL  = currentLabel['swallow pill'];
          ...
          // if navigation did not result from utterances,
          // go a level up and try events
          _events(currentLabel.events, context);
          if(!(context.sessionAttributes.hasChanged('PILL') || context.sessionAttributes.hasChanged('LABEL'))){
