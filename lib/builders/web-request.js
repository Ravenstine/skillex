    ...
    let req = http.request(defaultOptions, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if(contentType && contentType.match(/(application|text)\/.*(xml|html)/)){
            // Parse as XML if response type matches.
            // Should also work for RSS.
            context.webResponse = parseXML(data);
          } else {
            // Otherwise, always attempt to parse JSON.
