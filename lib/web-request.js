...
module.exports = (webRequest, context) => {
  ...
  if(typeof webRequest === 'string') {
    ...
    console.log(evalString(webRequest.url, context, {uriEncode: true}));
    uri = parseURI(evalString(webRequest.url, context, {uriEncode: true}));
    ...
    let req = http.request(defaultOptions, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if(webRequest["none dialog"] && context.webResponse === void(0)){
