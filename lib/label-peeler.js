...
module.exports = (label, context) => {
  ...
  .then(scriptResult => {
    ...
    // go to:
    if(label.hasOwnProperty('go to')) context.navigator.goTo(label['go to']);
    // go to state:
    if(label.hasOwnProperty('go to state')) context.navigator.goToState(label['go to state']);
    // if (true|false):
